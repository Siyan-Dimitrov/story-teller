...
def render_short(
    ...
    """Render one vertical short. Prefers cutting from final.mp4 (cheap, keeps
    music + motion); falls back to re-rendering from the scene's stills."""
    want = source or config.SHORT_SOURCE
    final_exists = (project_dir / "final.mp4").exists()
    if want == "final" and final_exists and scenes is not None:
        try:
...
def render_short_from_stills(
    ...
    image_paths = [project_dir / p for p in (scene.get("image_paths") or [])]
